...
net = cv2.dnn.readNetFromTensorflow(r"C:\Users\renuk\OneDrive\รูปภาพ\Desktop\aicte\Human Pose Estimation using Machine Learning\graph_opt.pb")
...
# Load the image from file uploader or use the default demo image
...
# Slider to adjust the threshold for keypoint detection
thres = st.slider('Threshold for detecting the key points', min_value=0, value=20, max_value=100, step=5)
thres = thres / 100  # Convert threshold to a float between 0 and 1

# Cache the pose detection function
@st.cache_data
...
# Detect pose and output the result
output = poseDetector(image)

# Display the output image with detected poses
